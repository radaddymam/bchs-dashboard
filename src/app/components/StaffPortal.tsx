...
function StaffLogin({ onLogin, deviceMode }: { onLogin: () => void; deviceMode: DeviceMode }) {
      ...
      <div className="flex-1 flex items-center justify-center px-5 py-10">
        <div className={`w-full ${isKiosk ? "max-w-lg" : "max-w-md"}`}>
          <div className="text-center mb-8">
            ...
            <h1 className={`font-display font-extrabold text-foreground mb-2 ${isKiosk ? "text-4xl" : "text-3xl"}`}>
              Staff Sign In
            </h1>
...
export function StaffPortal({ onExit, deviceMode }: { onExit: () => void; deviceMode: DeviceMode }) {
  ...
  if (!authed) {
    return <StaffLogin onLogin={() => setAuthed(true)} deviceMode={deviceMode} />;
  }
