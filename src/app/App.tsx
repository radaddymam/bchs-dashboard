...
type DeviceMode = "mobile" | "kiosk" | "desktop";
...
function CheckInScreen({ onCheckIn, deviceMode }: { onCheckIn: () => void; deviceMode: DeviceMode }) {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center px-5 py-10">
        <div className={`w-full ${isKiosk ? "max-w-lg" : "max-w-md"}`}>

          {/* Greeting */}
          <div className="text-center mb-8">
            <h1 className={`font-display font-extrabold text-foreground mb-2 ${isKiosk ? "text-5xl" : "text-4xl"}`}>
              Welcome
            </h1>
                ...
                <motion.div key="id" initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.14 }}>
                  ...
                  <button
                    ...
                    className={`w-full bg-primary text-primary-foreground rounded-xl font-semibold flex items-center justify-center gap-2 hover:opacity-90 active:scale-[0.98] transition-all ${
                      isKiosk ? "py-5 text-base" : "py-3.5 text-sm"
                    }`}
                  >
...
function Dashboard({ onSignOut, deviceMode }: { onSignOut: () => void; deviceMode: DeviceMode }) {
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto scrollbar-hide">
          <div className={`px-4 md:px-8 pt-5 pb-28 md:pb-8 mx-auto ${isKiosk ? "max-w-lg" : "max-w-2xl md:max-w-none"}`}>
            <AnimatePresence mode="wait">
              <motion.div
                key={tab}
        ...
        <div className="flex">
          {TABS.map(({ id, shortLabel, icon: Icon }) => (
            ...
            >
              ...
              <Icon size={isKiosk ? 24 : 20} strokeWidth={tab === id ? 2.5 : 1.6} />
              <span className={`font-medium ${isKiosk ? "text-sm" : "text-xs"}`}>{shortLabel}</span>
            </button>
          ))}
        </div>
      </nav>
...
function DeviceModeBar({ mode, onChange }: { mode: DeviceMode; onChange: (m: DeviceMode) => void }) {
  const modes: { id: DeviceMode; label: string; icon: React.ElementType }[] = [
    { id: "mobile",  label: "Mobile",  icon: Smartphone },
    { id: "kiosk",   label: "Kiosk",   icon: QrCode },
    { id: "desktop", label: "Desktop", icon: Monitor },
  ];
  return (
    <div className="fixed top-3 right-3 z-50 flex items-center gap-0.5 bg-foreground/88 backdrop-blur-sm rounded-full px-1 py-1 shadow-xl border border-white/8">
      {modes.map(({ id, label, icon: Icon }) => (
        <button
          key={id}
          onClick={() => onChange(id)}
          className={`flex items-center gap-1 px-2.5 py-1.5 rounded-full text-[11px] font-semibold transition-all duration-150 ${
            mode === id
...
function RoleLanding({ onPick, deviceMode }: { onPick: (r: "patient" | "staff") => void; deviceMode: DeviceMode }) {
  const isKiosk = deviceMode === "kiosk";
  const roles = [
    {
      id: "patient" as const,
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center px-5 py-10">
        <div className={`w-full ${isKiosk ? "max-w-2xl" : "max-w-xl"}`}>
          {/* Greeting */}
          <div className="text-center mb-10">
            <p className="text-xs font-semibold text-primary uppercase tracking-widest mb-3">
              Welcome
            </p>
            <h1 className={`font-display font-extrabold text-foreground mb-3 leading-none ${isKiosk ? "text-5xl" : "text-4xl"}`}>
              How can we help?
            </h1>
...
export default function App() {
  ...
  return (
    <div className="size-full relative">
      <DeviceModeBar mode={deviceMode} onChange={setDeviceMode} />
      ...
      <AnimatePresence mode="wait">
        ...
        {role === "patient" && (
          ...
          >
            {view === "checkin" ? (
              <CheckInScreen onCheckIn={() => setView("dashboard")} deviceMode={deviceMode} />
            ) : (
              <Dashboard onSignOut={() => setRole("landing")} deviceMode={deviceMode} />
            )}
          </motion.div>
        )}
        ...
        {role === "staff" && (
          <motion.div
            ...
            className="size-full"
          >
            <StaffPortal onExit={() => setRole("landing")} deviceMode={deviceMode} />
          </motion.div>
        )}
      </AnimatePresence>
    </div>
